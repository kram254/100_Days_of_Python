...
def account_reconciliation(filepath):
    transactions_data = pd.read_csv(filepath)
    transactions_data = transactions_data.sort_values(by=['Date'], ascending=True)
...
def fraud_detection(transactions_data):
    ...
    transactions_data['debit'] = transactions_data['Amount'].where(transactions_data['Amount'] < 0, 0)
    transactions_data['credit'] = transactions_data['Amount'].where(transactions_data['Amount'] > 0, 0)

    mean_debit_credit_ratio = transactions_data['debit'].mean() / transactions_data['credit'].abs().mean()
    std_debit_credit_ratio = transactions_data['debit'].std() / transactions_data['credit'].abs().std()
    ...
    fraud_data = transactions_data[((transactions_data['Running_Balance'] < (mean_balance - 2 * std_balance)) |
                                   ...
                                   ((transactions_data['debit'] / transactions_data['credit'].abs() > (mean_debit_credit_ratio + 2 * std_debit_credit_ratio)) |
                                    (transactions_data['debit'] / transactions_data['credit'].abs() < (mean_debit_credit_ratio - 2 * std_debit_credit_ratio)))]
    fraud_data = fraud_data[['ID_Number','First_Name','Last_Name','Address','Date','Amount','debit','credit','Running_Balance','Description']]
    return fraud_data
...
def compliance_reporting(transactions_data, fraud_data):
    compliant_data = transactions_data[~transactions_data.index.isin(fraud_data.index)]    
    report_data = pd.concat([compliant_data.describe(), fraud_data.describe()], axis=1, keys=['Compliant Transactions', 'Fraudulent Transactions'])
